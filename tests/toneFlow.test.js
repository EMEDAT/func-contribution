...
describe('ToneFlow API Tests', () => {
  ...
  it('responds professionally for account help', async () => {
    const res = await request(app).post('/functions/toneFlow').send({
      input: { message: "Can I get assistance with my account?", tone: "professional" }
    });
  ...
  it('responds friendly for account help', async () => {
    const res = await request(app).post('/functions/toneFlow').send({
      input: { message: "Can I get assistance with my account?", tone: "friendly" }
    });
    expect(res.statusCode).toBe(200);
    expect(res.body.output.response).toBe("Hey there! We're here to help you with your account. Just let us know what you need! 😊");
  });

  it('responds empathetically to general inquiry', async () => {
    const res = await request(app).post('/functions/toneFlow').send({
      input: { message: "I have a general question.", tone: "empathetic" }
    });
    expect(res.statusCode).toBe(200);
    expect(res.body.output.response).toBe("I understand how you feel and appreciate your patience. We're here to support you.");
  });

  it('handles missing tone gracefully', async () => {
    const res = await request(app).post('/functions/toneFlow').send({
      input: { message: "Can I get assistance with my account?" }
    });
    expect(res.statusCode).toBe(400);
    expect(res.body.output.error).toBe("Both 'message' and 'tone' are required in the input object.");
  });

  it('returns default response for unsupported tone', async () => {
    const res = await request(app).post('/functions/toneFlow').send({
      input: { message: "Can I get assistance with my account?", tone: "unsupported_tone" }
    });
    expect(res.statusCode).toBe(200);
    expect(res.body.output.response).toBe("Thank you for your message. We'll do our best to assist you.");
  });

  it('returns concise response to inquiry status', async () => {
    const res = await request(app).post('/functions/toneFlow').send({
      input: { message: "What's the status of my inquiry?", tone: "concise" }
    });
    expect(res.statusCode).toBe(200);
    expect(res.body.output.response).toBe("Message received. We'll respond shortly.");
  });

  it('responds supportively for account help', async () => {
    const res = await request(app).post('/functions/toneFlow').send({
      input: { message: "Can I get assistance with my account?", tone: "supportive" }
    });
    expect(res.statusCode).toBe(200);
  ...
  it('responds casually for account help', async () => {
    const res = await request(app).post('/functions/toneFlow').send({
      input: { message: "Can I get assistance with my account?", tone: "casual" }
    });
    expect(res.statusCode).toBe(200);
